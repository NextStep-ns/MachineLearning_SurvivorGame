...
class GameAI:
    ...
    def __init__(self):
        ...
        self.COW_REWARD=COW_REWARD
        self.KNIFE_REWARD=KNIFE_REWARD
...
#-----------------------------------------------------------------------------------------------------------------------
    ...
    def update_screen(self):
        ...
        knives_text = self.font.render(f"Knives: {self.n_knife}", True, (0, 0, 0))
        self.screen.blit(knives_text, (0, 75))  # Display at (0, 50) on the screen
        cows_text = self.font.render(f"Cows: {self.n_cow}", True, (0, 0, 0))
        ...
        if not getattr(self, exist_attr):
            for _ in range(Nbr):
                while True:
                    ...
                    x = random.randint(288, 550) - 1 # random.randint(0, len(self.sand_matrix) * 16) - 1
                    y = random.randint(288, 550) - 1 #random.randint(0, len(self.sand_matrix) * 16) - 1
        ...
        for obj in getattr(self, f"{item}_group"):
            if item == "cow":
                ...
                if obj.rect.colliderect(self.player.rect) and action[-1]==1:
                    setattr(self, f"interaction_{item}", True)
                    self.reward+=self.CARROT_REWARD
                    self.n_carrots+=1
                    return obj
