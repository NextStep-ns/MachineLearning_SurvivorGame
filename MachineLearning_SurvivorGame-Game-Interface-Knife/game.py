...
class Game:
    ...
    def __init__(self):
        ...
        #Initialize knife
        self.knife_group = pyscroll.PyscrollGroup(map_layer=map_layer, default_layer=2)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    # Check if the player is near the knife and press Enter to pick it up
                    for knife in self.knife_group:
                        if knife.is_near(self.player):
                            knife.pick_up(self.player, event.key)
    ...
    def spawn_knife(self):
        if not self.Knife_exist:
            while True:
                ...
                y = random.randint(0, len(self.sand_matrix)*16)
                x_m,y_m=self.game_to_matrix_position(x, y)
                knife_rect = pygame.Rect(x, y, 20, 20)
                if not (any(knife_rect.colliderect(wall) for wall in self.walls)) and (self.sand_matrix[x_m][y_m]>=10):
                    break
